...
tic
models = {'freedriftst'};                       % Fit models
subjects = 206; %[101 103 105 107 109 202 204 208 210 301 302 303 304 306 401 402 403 404]; % These need to match datafiles in the Data folder
for sidx = 1:numel(subjects)
    for midx = 1%:numel(models)
        ...
        %% Load subject data
        [data, cols] = loadFacerulesData(subject);
               
        %% Load parameters for inference
        [parms, hyperparms, thetaprior] = loadFreeDriftParmSettings(model);
        ...
        %% Load the exisiting sampling file if it exists; otherwise initialize chains
        if exist(fn, 'file') ~= 2
...
%             try parpool('MJSProfile1', n.chains); catch; parpool('local'); end % Open parallel job
            ...
            for i = 1:n.chains
                while weight(i,1) == -Inf % Ensure valid parms
                    ...
                    weight(i,1) = logDensLikeFD(getChain(theta, i, 1, 'one'), data, model); % Get likelihood
                end
            end
            starti = 2;
            
            t = nan(n.mc, 1); % Initialize a variable to keep track of time
            t(1) = toc;       % Initialization time
        else
%             try parpool('MJSProfile1', n.chains); catch; parpool('local'); end % Open parallel job
            parpool('local')
            load(fn, 'i', 'use', 'theta', 'weight', 't'); % Load existing varaibles
