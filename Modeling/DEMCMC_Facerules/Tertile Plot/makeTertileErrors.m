...
%         getsim = @(j,k,p)(prctile(sim.rt{j,k}(sim.resp{j,k} == cr(j) & sim.rt{j,k} < 3), p));
        ...
        simePercent{1}(j,:) = cell2mat(arrayfun(@(x)getSimPercentiles(x), 1:20, 'UniformOutput', false));
            
        getSimPercentiles = @(k)getesim(j,k,50);
        simePercent{2}(j,:) = cell2mat(arrayfun(@(x)getSimPercentiles(x), 1:20, 'UniformOutput', false));
        
        getSimPercentiles = @(k)getesim(j,k,75);
        simePercent{3}(j,:) = cell2mat(arrayfun(@(x)getSimPercentiles(x), 1:20, 'UniformOutput', false));   
