...
debug = true; % If debug == true, do experiment with reduced number of trials
...
outputfile = sprintf('2015_comptask_s%03d.dat', subject);
csvoutputfile = sprintf('2015_comptask_s%03d.csv', subject);
dataFolder = fullfile(pwd, 'Data');
%
%trialdatafile = ['temp_' csvoutputfile];
...
for bidx = 1:nBlocks
    ...
    for i = 1:nTrialsPerBlock        
        ...
        Priority(0);
        %To save after each trial (incomplete)
        %trialoutput = [subject, bidx, i, response(i,1), correctFlag(i,1), rt(i,1)];
        %dlmwrite(trialdatafile, trialoutput, '-append')
        %
    end
